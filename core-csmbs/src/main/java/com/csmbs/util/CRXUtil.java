...
public final class CRXUtil {
	 ...
	 * @param serviceClass the class whose reference has to be returned.
	 * @param request as SlingHttpServletRequest.
	...
	public static <T> T getServiceReference(final Class<T> serviceClass, final SlingHttpServletRequest request) {
		LOGGER.info(":Inside getServiceReference() --> ");
		final SlingBindings bindings = (SlingBindings) request.getAttribute(SlingBindings.class.getName());
		/**
		 * In case SlingBindings is not available in request attribute map. Find it in the BundleContext. If this
		 * request is not originated from script i.e jsp then bindings are not set in request. Other possible way to get
		 * SlingBindings initialized is to set the SlingBindings in the request in the jsp itself, while making a call
		 * to the Servlet.
		 */
		T serviceRef;
		if (bindings == null) {
			serviceRef = getServiceReference(serviceClass);
		} else {
			serviceRef = bindings.getSling().getService(serviceClass);
		}
		LOGGER.info(":Exiting getServiceReference() --> ");
		return serviceRef;
	}
